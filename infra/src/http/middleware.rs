...
pub async fn authorized_user_middleware(
    ...
    let token = match get_access_token_from_request(&mut request).await {
        Some(token) => token,
        None => {
...
async fn get_access_token_from_request(request: &mut Request) -> Option<SecretString> {
    // クッキーからアクセストークンを取得
    if let Some(cookie) = request.extensions().get::<CookieJar>() {
        if let Some(cookie_value) = cookie.get(COOKIE_ACCESS_TOKEN_KEY) {
